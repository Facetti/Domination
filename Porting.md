...
- Syntax ***d_smm|N1|Mission|...*** example: d_smm|21|stealflag|flag|1|smarmor|1|2|1|1|400|true|sminf|1|2|300|false
	...
	- ***Mission - some have sub parameters 4. ..*** 
		- `convoy|stop/start`
			- ***d_smm|32|convoy|start*** - Start point
			- ***d_smm|32|convoy|stop*** - End point
		- `stealflag|flag|#`	different types of flags ?
		- `tankdepot|tank|#`      number of tanks
		- `arrest` - a persoon
		- `artibase` - bring to base
		- `evac|time|#` - timer in minutes to solve the  mission
		- `radiotower` - destroy
		- `prisoners` - bring to base
		- `stealchopper` - bring to base
		- `stealtank` - bring to base
		- `stealapc` - bring to base
		- `stealplane` - bring to base
		- `specops|radius|#` - kill specops, postionied within radius
		- `eliminateofficer` - kill
		- `eliminategovmember` - kill
		- `eliminatesniper` - kill
		- `fuelstation` - destroy
		- `transformer` - destroy
		- `barracks` - destroy
		- `hangar` - destroy
	...
	- ***additional troups and vehicles***
...
DominationMissionGroup.vars delete it 
